...
class AgentOrchestrator:
    ...
    def __init__(self):
        ...
        # ⚡ LAZY-LOAD THESE (don't initialize on startup)
        self._rag = None
        self._cache = None
        self._bq = None
    ...
    def rag(self):
        ...
        if self._rag is None:
            ...
            try:
                ...
                print("✅ RAG loaded and connected to Pinecone")
    ...
    def bq(self):
        ...
        if self._bq is None:
            print("🔄 Loading BigQuery logger...")
            try:
                from utils.bigquery_logger import BigQueryLogger
                self._bq = BigQueryLogger(batch_size=5, flush_interval=10)
                if self._bq.enabled:
                    print("✅ BigQuery analytics enabled")
                else:
                    print("⚠️ BigQuery disabled (credentials not found)")
                    self._bq = None
    ...
    async def generate_code(
        ...
        print(f"🔍 DEBUG: Checking cache... self.cache = {self.cache}")
        if self.cache:  # This triggers lazy loading
            print(f"🔍 DEBUG: Cache enabled, calling cache.get()")
            cached = self.cache.get(description, language)
            print(f"🔍 DEBUG: cache.get() returned: {cached is not None}")
            if cached:
                print(f"💰 CACHE HIT - Returning instant result, saved ${cached.get('cost', 0):.4f}!")
                ...
                if websocket_callback:
                    for agent_id in ['requirements', 'programmer', 'test_designer', 'test_executor', 'documentation']:
                        await self._send_update(websocket_callback, agent_id, "completed", 100, "Retrieved from cache ⚡")
        ...
        if needs_review:
            ...
            print(f"🚨 HITL REVIEW REQUIRED: {' | '.join(reason)}")
        ...
        print(f"🔍 DEBUG: self.bq = {self.bq}, type = {type(self.bq)}")
        if self.bq:
            print(f"🔍 DEBUG: About to call log_generation with generation_id={generation_id}")
